...
const inputSurname = form.children[1];

isValidData(inputName, inputSurname);



function isValidData(inputName, inputSurname) {
    inputName.addEventListener("keydown", (e)=>{
        let inputValue = e.target.value.length + 1;
        
        if(inputValue < 5){
            e.target.style.background = "red";
            console.log("Ur data is incorrect");
    
        } else {
            e.target.style.background = "lightgreen";
            console.log("GJ all is correct!");
        }
    })
